...
class IRListenerManager:
    ...
    async def start_listening(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(
                f"Connected to pigpiod, setting up IR receiver on GPIO{self.PIN}"
            )
    ...
    async def stop_listening(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(
                f"IR listener stopped successfully. Captured {events_count} events total"
            )
            return (
                True,
    ...
    def _gpio_callback(self, gpio, level, tick):
        ...
        try:
            ...
            if self._signal_counter % 100 == 0:
                logger.debug(
                    f"GPIO callback executing: gpio={gpio}, level={level}, tick={tick}"
                )
    ...
    def _complete_signal_sync(self):
        ...
        try:
            with self._signal_lock:
                ...
                current_time = time.time()
                time_since_last = (
                    current_time - self._last_signal_time
                ) * 1000  # Convert to ms

                if time_since_last >= self._signal_timeout_ms:
                    logger.debug(
                        f"Completing signal after {time_since_last:.1f}ms timeout"
                    )
    ...
    def _finish_current_signal(self):
        ...
        try:
            ...
            if self._enable_signal_matching:
                ...
                if matched:
                    logger.info(
                        f"Signal {signal_number} matches previous: {matched['code']}"
                    )
                    logger.debug(
                        f"Match details: new_pulses={len(timing_data)}, "
                    ...
                    logger.debug(
                        f"Signal {signal_number} is unique, storing as new pattern"
                    )
                    self._store_recent_signal(timing_data, analysis)
            else:
                # TEMPORARY: Matching disabled - treat every signal as unique
                logger.debug(
                    f"Signal {signal_number} treated as unique (matching disabled to prevent false positives)"
                )
                self._store_recent_signal(timing_data, analysis)

                # For debugging: show what would have matched
                if (
                    len(self._recent_signals) > 1
                ):  # Only check if we have signals to compare
                    potential_match = self._find_matching_signal(timing_data)
                    if potential_match:
                        logger.warning(
                            f"Signal {signal_number} WOULD HAVE matched: {potential_match['code']} "
            ...
            logger.info(
                f"IR signal analysis complete: code={analysis.get('code', 'N/A')}, "
            ...
            if len(self._ir_events) > self._max_events:
                ...
                logger.debug(
                    f"Trimmed {remove_count} old events, {len(self._ir_events)} remaining"
                )

            logger.info(
                f"Signal {signal_number}: {analysis.get('protocol', 'Unknown')} - {analysis.get('code', 'N/A')}"
            )

        except Exception as e:
            logger.error(
                f"Signal {signal_number} processing failed: {e}", exc_info=True
            )
            ...
            ir_event = {
                ...
                "analysis": {
                    "protocol": "Raw",
                    "code": f"0xRAW{signal_number:04X}",
                    "error": str(e),
                },
            }
            self._ir_events.append(ir_event)
    ...
    def _analyze_signal(
        ...
        logger.debug(
            f"Generated guaranteed unique code: timing=0x{timing_hash:04X}, state=0x{state_hash:04X}, "
    ...
    def _find_matching_signal(
        self, timing_data: List[Tuple[str, int]]
    ) -> Optional[Dict]:
        ...
        normalized = []
        for state, duration in timing_data[
            :20
        ]:  # Use more pulses for better fingerprinting
        ...
        for recent in self._recent_signals[-3:]:
            if self._patterns_match(normalized, recent["normalized"]):
                ...
                if time_since_recent < 5.0:
                    logger.debug(
                        f"Signal matches recent pattern from {time_since_recent:.1f}s ago"
                    )
                    return recent["analysis"]
                else:
                    logger.debug(
                        f"Signal pattern matched but too old ({time_since_recent:.1f}s), treating as new"
                    )

        return None

    def _patterns_match(
        self, pattern1: List[int], pattern2: List[int], tolerance: float = 0.15
    ) -> bool:
        """Check if two timing patterns match within tolerance - stricter matching to prevent false positives."""
        if (
            abs(len(pattern1) - len(pattern2)) > 2
        ):  # Allow small length differences but not large ones
            return False
            ...
            if p1 == 0 and p2 == 0:
                ...
                if diff <= tolerance:
                    ...
                    if diff > 0.5:  # More than 50% difference
                        logger.debug(
                            f"Large timing difference detected: {p1}μs vs {p2}μs ({diff:.2%})"
                        )
                        return False

        if (
            significant_pulses < 6
        ):  # Need enough significant pulses for reliable matching
            return False

        match_rate = matches / significant_pulses
        logger.debug(
            f"Pattern match rate: {match_rate:.2%} ({matches}/{significant_pulses} pulses)"
        )
    ...
    def _store_recent_signal(self, timing_data: List[Tuple[str, int]], analysis: Dict):
        ...
        if len(self._recent_signals) > 10:  # Reduced from max_recent_signals
            ...
            logger.debug(
                f"Trimmed {remove_count} old recent signals, {len(self._recent_signals)} remaining"
            )
    ...
    def get_recent_events(self, horizon_s: int = 20) -> List[Dict]:
        ...
        logger.info(
            f"Retrieved {len(recent_events)} IR events from last {horizon_s} seconds"
        )
    ...
    def clear_events(self):
        ...
        logger.info(
            f"Cleared {events_count} IR events and {recent_count} recent signal patterns from memory"
        )
    ...
    def get_listener_status(self) -> Dict:
        ...
        status = {
            ...
            "listener_task_active": self._is_listening
            and (self._pi.connected if self._pi else False),
        ...
        if self._ir_events:
            ...
            status["latest_event_code"] = latest_event.get("analysis", {}).get(
                "code", "N/A"
            )

        # Log the status for debugging
        logger.info(
            f"IR listener status: listening={status['is_listening']}, "
