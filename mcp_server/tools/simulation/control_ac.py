    ...
    async def execute(self, input_data: ControlACInput) -> ToolResponse:
        ...
        try:
            ...
            if not success:
                return ToolResponse.from_model(
                    ControlACOutput(
                        success=False, message="Failed to read sensor", ac_running=False
                    )
                )
            ...
            if input_data.action == "turn_on":
                ...
                logger.info(
                    f"AC cooling: {temp_f:.1f}°F → {new_temp:.1f}°F (target: {input_data.target_temp_f}°F)"
                )
                return ToolResponse.from_model(
                    ControlACOutput(
                        success=True,
            ...
            return ToolResponse.from_model(
                ControlACOutput(
                    success=False, message=f"Failed: {str(e)}", ac_running=False
                )
            )
