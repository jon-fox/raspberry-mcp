...
def save_device_mapping(
    ...
    try:
        ...
        if ir_events:
            ...
            device_mapping["tx_device"] = first_event.get(
                "tx_device", "raspberry_pi_ir"
            )
        ...
        for i, (operation, event) in enumerate(zip(all_operations, ir_events)):
            ...
            operation_data = {
                ...
                "raw_timing_data": analysis.get(
                    "raw_timing_data", event.get("timing_data", [])
                ...
                "captured_at": (
                    ...
                    if hasattr(event.get("timestamp", datetime.now()), "isoformat")
                    else str(event.get("timestamp", datetime.now()))
                ),
            }
...
def get_device_operation_details(
    device_key: str, operation: str
) -> Dict[str, Any] | None:
